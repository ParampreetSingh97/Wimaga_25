...
from keras.layers import Input, Dense, TimeDistributed, Lambda, Activation, ZeroPadding1D, Cropping1D, SpatialDropout1D, MaxPooling1D, UpSampling1D, Conv1D, LSTM, multiply, add, concatenate, Layer
...
class PeriodicPadding1D(Layer):
    def __init__(self, padding=(1, 1), **kwargs):
        super(PeriodicPadding1D, self).__init__(**kwargs)
    ...
    def get_config(self):
        config = super(PeriodicPadding1D, self).get_config()
...
def temporal_convs_linear(n_nodes, conv_len, n_classes, n_feat, max_len, 
    ...
    if causal: 
        model = PeriodicPadding1D((conv_len // 2, 0))(inputs)
    else:
        model = inputs
    model = Conv1D(n_nodes, conv_len, padding='same', activation='relu')(model)
    if causal: 
        model = Cropping1D((0, conv_len // 2))(model)
...
def ED_TCN(n_nodes, conv_len, n_classes, n_feat, max_len, 
    ...
    for i in range(n_layers):
        if causal: model = PeriodicPadding1D((conv_len // 2, 0))(model)
...
def ED_TCN_atrous(n_nodes, conv_len, n_classes, n_feat, max_len, 
    ...
    for i in range(n_layers):
        if causal: model = PeriodicPadding1D((conv_len // 2, 0))(model)
        model = Conv1D(n_nodes[i], conv_len, dilation_rate=i+1, padding='same')(model)
        if causal: model = Cropping1D((0, conv_len // 2))(model)
    ...
    for i in range(n_layers):
        if causal: model = PeriodicPadding1D((conv_len // 2, 0))(model)
        model = Conv1D(n_nodes[-i-1], conv_len, dilation_rate=n_layers-i, padding='same')(model)      
        if causal: model = Cropping1D((0, conv_len // 2))(model)
...
def Dilated_TCN(num_feat, num_classes, nb_filters, dilation_depth, nb_stacks, max_len, 
    ...
    def residual_block(x, s, i, activation):
        ...
        if causal:
            x = PeriodicPadding1D(((2**i) // 2, 0))(x)
            conv = Conv1D(nb_filters, 2, dilation_rate=2**i, padding='same',
                          name='dilated_conv_%d_tanh_s%d' % (2**i, s))(x)
            conv = Cropping1D((0, (2**i) // 2))(conv)
        else:
            conv = Conv1D(nb_filters, 3, dilation_rate=2**i, padding='same',
                          name='dilated_conv_%d_tanh_s%d' % (2**i, s))(x)                                        
    ...
    x = input_layer
    if causal:
        x = PeriodicPadding1D((1, 0))(x)
        x = Conv1D(nb_filters, 2, padding='same', name='initial_conv')(x)
        x = Cropping1D((0, 1))(x)
    else:
